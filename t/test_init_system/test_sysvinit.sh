...
function test_execute_valid()
{
    ...
    OUTPUT=$(execute_service "start" "EXAMPLE-SERVICE" 2>&1)
    if [[ "$OUTPUT" == *"Success"* ]]; then
        pass "execute_service with valid command"
    else
        fail "execute_service failed on valid command"
    fi
}
...
function test_execute_invalid()
{
    ...
    if OUTPUT=$(execute_service "invalid-cmd" "EXAMPLE-SERVICE" 2>&1); then
        fail "execute_service accepted invalid command"
    else
        if [[ "$OUTPUT" == *"Unknown command"* ]]; then
            pass "execute_service rejected invalid command"
        else
...
function main()
{
	test_wrapper start_service
	test_wrapper stop_service
	test_wrapper force_reload_service
	test_wrapper restart_service
	test_wrapper status_service
	test_execute_valid
